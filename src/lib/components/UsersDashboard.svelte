<script>
    ...
    let filteredSubscriptions = $derived(
        userSubscriptions.filter(s => 
            (s.event?.name && s.event.name.toLowerCase().includes(subSearchQuery.toLowerCase())) ||
            (s.subscription_type && s.subscription_type.toLowerCase().includes(subSearchQuery.toLowerCase()))
        )
    );
    ...
    async function fetchSubscriptions(userId) {
        try {
            ...
            const response = await fetch(`${API_URL}/v1/subscriptions?user_id=${userId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    Accept: "application/json",
                },
    ...
    async function saveSubscription() {
        try {
            const response = await fetch(`${API_URL}/v1/subscriptions/${editingSubscription.id}`, {
                method: "PUT",
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            if (response.ok) {
                ...
                const errorMessage = errorData.errors
                    ...
                    : errorData.message;
                showModal("error", errorMessage || "Erro ao atualizar inscrição.");
            }
        } catch (err) {
...
<div class="space-y-8 animate-fade-in">
        ...
        <div>
            ...
            {:else if filteredUsers.length === 0}
                <div class="text-center py-24 bg-bg-secondary/30 rounded-[3rem] border-2 border-dashed border-border-ui uppercase tracking-widest">
                    <p class="text-text-secondary text-xs font-bold mb-6">Nenhum usuário encontrado</p>
                </div>
            {:else}
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-8 text-text-primary">
                    {#each filteredUsers as user}
                        <div class="bg-bg-secondary border border-border-ui p-6 rounded-[2.5rem] hover:shadow-2xl hover:-translate-y-1 transition-all group">
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <h3 class="text-xl font-black group-hover:text-brand transition-colors">{user.name}</h3>
                                    <p class="text-text-secondary text-sm mt-1">{user.email}</p>
                                </div>
                                <div class="px-3 py-1 bg-text-primary/5 text-text-secondary rounded-full text-[10px] font-bold uppercase tracking-wider">
                                    {formatCPF(user.cpf) || 'Sem CPF'}
                                </div>
                            </div>
                            <div class="flex justify-end pt-4 border-t border-border-ui">
                                <button
    ...
    {:else}
        <div>
            ...
            <div class="bg-bg-secondary border border-border-ui p-8 rounded-[2.5rem] mb-8">
                <h2 class="text-2xl font-black mb-6">Informações do Usuário</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold mb-1">Nome</p>
                        <p class="font-medium">{selectedUser.name || '-'}</p>
                    </div>
                    <div>
                        <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold mb-1">Email</p>
                        <p class="font-medium">{selectedUser.email || '-'}</p>
                    </div>
                    <div>
                        <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold mb-1">CPF</p>
                        <p class="font-medium">{formatCPF(selectedUser.cpf)}</p>
                    </div>
                    <div>
                        <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold mb-1">Telefone</p>
                        <p class="font-medium">{formatPhone(selectedUser.phone)}</p>
                    </div>
                </div>
            </div>
            ...
            {#if loadingSubscriptions}
                <div class="flex flex-col items-center justify-center py-10 text-text-secondary">
                    <div class="w-8 h-8 border-[3px] border-brand border-t-transparent rounded-full animate-spin mb-4"></div>
                    <p class="animate-pulse text-sm">Carregando inscrições...</p>
                </div>
            {:else if filteredSubscriptions.length === 0}
                <div class="text-center py-12 bg-bg-secondary/30 rounded-[2rem] border-2 border-dashed border-border-ui uppercase tracking-widest">
                    <p class="text-text-secondary text-xs font-bold">Nenhuma inscrição encontrada</p>
                </div>
            {:else}
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-8 text-text-primary">
                    {#each filteredSubscriptions as sub}
                        <div class="bg-bg-secondary border border-border-ui p-6 rounded-[2rem] relative">
                            <h3 class="text-xl font-black mb-4">{sub.event?.name || 'Inscrição'}</h3>
                            <div class="grid grid-cols-2 gap-4 mb-6">
                                <div>
                                    <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold mb-1">Status de Pagamento</p>
                                    <p class="font-medium text-sm">{sub.paid_the_fee ? 'Confirmado' : 'Pendente'}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold mb-1">Tipo</p>
                                    <p class="font-medium text-sm">{sub.subscription_type || '-'}</p>
                                </div>
                                {#if sub.event?.eventable_type !== "App\\Models\\Festival"}
                                    <div>
                                        <p class="text-[10px] text-text-secondary uppercase tracking-widest font-bold mb-1">Sorteado</p>
                                        <p class="font-medium text-sm">
                                            <span class="px-2 py-0.5 rounded text-xs font-bold {sub.was_selected ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}">
...
{#if editingSubscription}
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
        <div class="bg-bg-secondary w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-border-ui transform transition-all">
            <div class="p-8">
                ...
                <div class="space-y-6">
                    {#if editingSubscription.event?.eventable_type === "App\\Models\\Festival"}
                        <div>
                            <label class="block text-xs font-bold uppercase tracking-widest text-text-secondary mb-2" for="subscription_type">
                                Tipo de Inscrição (Festival)
                            </label>
                            <select
                                id="subscription_type"
                                bind:value={editingSubscription.subscription_type}
                        ...
                        <div class="flex items-center gap-4 p-4 bg-bg-primary border-2 border-border-ui rounded-2xl">
                            <input
                                id="paid_the_fee"
                                type="checkbox"
                            ...
                            />
                            <label class="text-sm font-bold cursor-pointer select-none flex-grow" for="paid_the_fee">
                                Taxa Paga?
                            </label>
                        </div>
                    {:else}
                        ...
                        <div class="flex items-center gap-4 p-4 bg-bg-primary border-2 border-border-ui rounded-2xl">
                            <input
                                ...
                                class="w-5 h-5 accent-brand rounded"
                            />
                            <label class="text-sm font-bold cursor-pointer select-none flex-grow" for="was_selected">
